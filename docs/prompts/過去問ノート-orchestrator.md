...
- 複数問は **必ずサブエージェント**で進める（**1問 = 1 Task**。親が複数問を連続で自分で書く **親一括** は、下記「親一括」のとおりユーザー明示時のみ）。
- **Task を並列起動**（複数問を1 Task にまとめない — Cursor 停止の主因）。
- 同時起動は **最大2問**（4問並列は避ける）。
...
**既定では行わない。** 件数が多くても、品質と手順の隔離のため **1問1 Task・並列2** のサブエージェント運用を続ける。
...
停止や失敗時は、まず **並列2のまま件数を減らす（例: 10問ずつ）**・**該当問だけワーカー再実行**を検討し、親一括が必要ならユーザーに確認する。
